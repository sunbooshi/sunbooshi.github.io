...
当我们将Xbox手柄接入之后会xpad在`/dev/input`下创建两个文件`/dev/input/event0`和`/dev/input/js0`（注意也可能是event1之类的），其中的js0设备是摇杆设备（joystick）这个设备可以获取摇杆数据但是按键数据获取不到，event0设备可以获取所有按键。所以想要获取Xbox手柄的按键数据只要读取event0文件即可。这个event0就是Linux下的标准输入设备，每一次按键事件都会被封装为`struct input_event`结构，其定义如下：
...
当我们读取event0时，实际上获取的内容就是一个`struct input_event`，这里我们主要关注type、code和value即可。（关于type、code的详细定义可以参考[这里](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/include/uapi/linux/input-event-codes.h)）。

在处理数据之前，先了解一下Xbox的各个按键，如下图所示:

![XboxGamepad](/images/xboxgamepad.png)
...
项目地址：[https://github.com/sunbooshi/xboxinput](https://github.com/sunbooshi/xboxinput)
